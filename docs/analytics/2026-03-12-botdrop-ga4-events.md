# Koda Android GA4 Event Dictionary

Date: 2026-03-12
Application ID: `dev.koda`
Analytics backend: Firebase Analytics / GA4

## Scope

This document covers the Koda Android UI instrumentation added in the launcher, setup funnel, dashboard, automation panel, and the integrated Shizuku `manager` flows.
...
```bash
adb shell setprop debug.firebase.analytics.app dev.koda
```
...
1. Confirm the app uses the `dev.koda` package on device
