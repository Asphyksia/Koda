...
Goal: let Koda/OpenClaw reliably control the device UI on a dedicated phone without root, using a structured (selector-based) API instead of coordinate clicking.
...
1. **AccessibilityService** (`KodaAccessibilityService`)
...
2. **Automation Controller Service** (`AutomationControllerService`, foreground)
   ...
   - Translates API calls into binder calls on `KodaAccessibilityService`.
