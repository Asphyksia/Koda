...
- Before running any `npm view` / `npm install -g ...` in Termux, Koda runs an auto-selection snippet.
- If `BOTDROP_NPM_REGISTRY` is set, it wins (explicit override).
- Otherwise Koda detects **current network** via default gateway IP (`ip route`), and caches a chosen registry for that gateway for 24h.
...
- When cache exists for current gateway, Koda re-validates that registry; stale cache is automatically re-resolved.
