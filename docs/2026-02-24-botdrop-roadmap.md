...
- 状态：**未开始**
- 当前界面直接复制自官方 Shizuku Manager，需改为 Koda 风格。
- 动作项：
  - 重构文案、图标、按钮布局为 Koda 统一风格；
...
- [ ] 使用 `KodaService.updateOpenclaw("openclaw@x.y.z")` 执行安装。
...
| 项 | 说明 |
|---|---|
| **免费 LLM API 额度** | Koda 提供免费试用额度（代理调用 Claude/GPT），用户无需注册第三方 API Key 即可开始 |
