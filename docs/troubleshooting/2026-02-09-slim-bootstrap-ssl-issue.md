...
2. **CA certificate check:**
   ```bash
   ls -la /data/data/dev.koda/files/usr/etc/tls/
   # Result: cert.pem (225KB) EXISTS

   ls -la /data/data/dev.koda/files/usr/etc/ssl/certs/
...
4. **Hypothesis test:**
   ```bash
   export SSL_CERT_FILE=/data/data/dev.koda/files/usr/etc/tls/cert.pem
...
- Removed the configuration that sets `SSL_CERT_FILE=/data/data/dev.koda/files/usr/etc/tls/cert.pem`
...
Modify `KodaService.java` executeCommandSync():
```java
