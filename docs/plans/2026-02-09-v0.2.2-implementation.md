# Koda v0.2.2 Features Implementation Plan
...
**Tech Stack:** Android Java, SharedPreferences, RecyclerView, KodaService for command execution.
...
- `app/src/main/java/app/botdrop/KodaService.java`
- `app/src/main/java/app/botdrop/KodaConfig.java`
...
public class ConfigTemplateCache {
    ...
    public static boolean applyTemplate(Context ctx, ConfigTemplate template) {
        ...
        try {
            ...
            boolean success = KodaConfig.setProvider(template.provider, template.model);
            if (!success) {
            ...
            // Set API key
            success = KodaConfig.setApiKey(template.provider, template.apiKey);
            if (!success) {
            ...
            if (template.tgBotToken != null && !template.tgBotToken.isEmpty()) {
                success = KodaConfig.setTelegramChannel(template.tgBotToken, template.tgUserId);
                if (!success) {
...
```java
    private void onVerifySuccess() {
        ...
        boolean providerWritten = KodaConfig.setProvider(template.provider, model);
        if (!providerWritten) {
            showStatus("Failed to write configuration", false);
            return;
        }

        boolean keyWritten = KodaConfig.setApiKey(template.provider, template.apiKey);
        if (!keyWritten) {
...
```java
    private void onChangeModelClick() {
        ...
        ModelSelectorDialog dialog = new ModelSelectorDialog(this, mKodaService);
        dialog.show((provider, model) -> {
...
```java
    private boolean updateModelConfig(String fullModel) {
        try {
            ...
            // Write config
            return KodaConfig.writeConfig(config);
