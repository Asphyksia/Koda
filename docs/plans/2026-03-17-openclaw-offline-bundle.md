...
Run: `./gradlew :app:testDebugUnitTest --tests dev.koda.BundledOpenclawUtilsTest --no-daemon`
...
- Modify: `app/src/main/java/app/botdrop/KodaService.java`
- Modify: `app/src/main/java/app/botdrop/AgentSelectionFragment.java`
- Test: `app/src/test/java/app/botdrop/KodaServiceTest.java`
...
./gradlew :app:testDebugUnitTest \
  --tests dev.koda.KodaServiceTest \
  --tests dev.koda.OpenclawVersionUtilsTest \
  --no-daemon
```
...
- make `KodaService.installOpenclaw()` refresh scripts and stage assets before executing the script
- make `KodaService.updateOpenclaw()` reuse the same offline staged bundle when the bundled version is selected
...
```bash
git add app/src/main/java/com/termux/app/TermuxInstaller.java app/src/main/java/app/botdrop/KodaService.java app/src/main/java/app/botdrop/AgentSelectionFragment.java app/src/test/java/app/botdrop/KodaServiceTest.java app/src/test/java/app/botdrop/OpenclawVersionUtilsTest.java
...
Run: `./gradlew :app:testDebugUnitTest --tests dev.koda.ChannelSetupHelperTest --no-daemon`
...
```bash
adb shell run-as dev.koda ls -R files/usr/share/botdrop/offline-openclaw
adb shell logcat -d -v time | grep -E 'BundledOpenclaw|Koda\\.TermuxInstaller|Koda\\.KodaService|qqbot'
```
...
```bash
./gradlew :app:testDebugUnitTest \
  --tests dev.koda.BundledOpenclawUtilsTest \
  --tests dev.koda.KodaServiceTest \
  --tests dev.koda.OpenclawVersionUtilsTest \
  --tests dev.koda.ChannelSetupHelperTest \
  --no-daemon
```
