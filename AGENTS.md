...
- `app/`: main Koda Android app (UI, services, setup flow, resources, native bootstrap glue).
...
- Keep package structure consistent (`dev.koda.*`, `com.termux.*`).
- Class names use `PascalCase`; methods/fields use `camelCase`; constants use `UPPER_SNAKE_CASE`.
- Test classes end with `Test` (for example `KodaServiceTest`, `TerminalRowTest`).
