...
public class TermuxInstallerTest {
    ...
    @Test
    public void testBuildOpenclaudeInstallScriptBody_doesNotInstallQqbotPlugin() throws Exception {
        Method method = TermuxInstaller.class.getDeclaredMethod(
            "buildOpenclaudeInstallScriptBody",
            String.class,
            int.class
        );
        method.setAccessible(true);

        String script = (String) method.invoke(null, "openclaude@latest", 2048);

        assertTrue(script.contains("KODA_STEP:2:START:Installing OpenClaude"));
        assertTrue(script.contains("ln -sfn \"$TARGET_DIR\" \"$OFFLINE_CURRENT_LINK\""));
        assertTrue(script.contains("GLOBAL_NODE_MODULES_ROOT=\"/data/data/dev.koda/files/usr/lib/node_modules\""));
        ...
        assertFalse(script.contains("GLOBAL_OPENCLAUDE_LINK=\"/data/data/dev.koda/files/usr/lib/node_modules/openclaude\""));
