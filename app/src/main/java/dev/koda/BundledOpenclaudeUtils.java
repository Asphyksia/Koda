...
public final class BundledOpenclaudeUtils {

    public static final String ASSET_ROOT = "offline-openclaude";
    public static final String MANIFEST_ASSET_PATH = ASSET_ROOT + "/manifest.properties";
    public static final String DEFAULT_RUNTIME_ARCHIVE_NAME = "openclaude-runtime.tar";
    public static final String DEFAULT_QQBOT_DIR_NAME = "qqbot";
    public static final String STAGED_ROOT = TermuxConstants.TERMUX_PREFIX_DIR_PATH + "/share/koda/offline-openclaude";
    public static final String STAGED_RUNTIME_ROOT = TermuxConstants.TERMUX_PREFIX_DIR_PATH + "/share/koda/openclaude-runtime";
    ...
    public static final String STAGED_QQBOT_PLUGIN_DIR = TermuxConstants.TERMUX_HOME_DIR_PATH + "/.openclaude/extensions/qqbot";
    public static final String OFFLINE_QQBOT_INSTALL_SCRIPT_PATH =
        TermuxConstants.TERMUX_PREFIX_DIR_PATH + "/share/koda/install-qqbot-offline.sh";

    private BundledOpenclaudeUtils() {
    }
    ...
    public static String resolvePreferredInstallSpec(
        ...
        String normalizedRequested = OpenclaudeVersionUtils.normalizeInstallVersion(requestedInstallSpec);
        if (manifest == null) {
            return normalizedRequested;
        }

        if (normalizedRequested == null || "openclaude@latest".equals(normalizedRequested)) {
    ...
    public static String buildOfflineQqbotInstallScriptBody() {
        return "#!" + TermuxConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/bash\n"
            ...
            + "CONFIG_PATH=\"" + TermuxConstants.TERMUX_HOME_DIR_PATH + "/.openclaude/openclaude.json\"\n"
            + "NODE_BIN=\"" + TermuxConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/node\"\n"
            + "if [ ! -f \"$QQBOT_SOURCE/openclaude.plugin.json\" ]; then\n"
            + "  echo \"KODA_ERROR:Bundled QQ Bot plugin source is missing\"\n"
            + "  exit 1\n"
            + "fi\n"
            + "rm -rf \"$QQBOT_TARGET\"\n"
            + "mkdir -p \"$QQBOT_TARGET\"\n"
            + "cp -R \"$QQBOT_SOURCE/.\" \"$QQBOT_TARGET/\"\n"
            + "if [ ! -f \"$QQBOT_TARGET/openclaude.plugin.json\" ]; then\n"
            + "  echo \"KODA_ERROR:Bundled QQ Bot plugin copy failed\"\n"
            + "  exit 1\n"
            + "fi\n"
            ...
            + " \"$CONFIG_PATH\" || {\n"
            + "    echo \"KODA_ERROR:Failed to register QQ Bot plugin in openclaude.json\"\n"
            + "    exit 1\n"
            + "  }\n"
            + "fi\n"
