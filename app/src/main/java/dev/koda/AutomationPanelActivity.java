...
public class AutomationPanelActivity extends Activity {
    ...
    private static final String U2_AUTOMATOR_APT_PACKAGE = "python-uiautomator2-koda";
            ...
            new rikka.shizuku.Shizuku.OnRequestPermissionResultListener() {
                ...
                public void onRequestPermissionResult(int requestCode, int grantResult) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        mStatusText.setText(
                                granted
                                        ? getString(R.string.koda_shizuku_permission_granted)
                                        : getString(R.string.koda_shizuku_permission_denied)
                        );
                        Toast.makeText(
    ...
    private void bindKodaService() {
        Intent intent = new Intent(this, KodaService.class);
        try {
            bindService(intent, mConnection, BIND_AUTO_CREATE);
        } catch (Throwable e) {
            Logger.logWarn(LOG_TAG, "Failed to bind KodaService: " + e.getMessage());
        }
    }
    ...
    private void diagnoseShizukuPermission() {
        ...
        if (!Shizuku.pingBinder()) {
            mStatusText.setText(getString(R.string.koda_shizuku_binder_not_ready));
            Toast.makeText(this, getString(R.string.koda_shizuku_binder_not_ready), Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (permission == PackageManager.PERMISSION_GRANTED) {
            mStatusText.setText(getString(R.string.koda_shizuku_permission_already_granted));
            Toast.makeText(this, getString(R.string.koda_shizuku_permission_already_granted), Toast.LENGTH_SHORT).show();
            runShizukuGatewayPrecheck();
            return;
        }

        runShizukuPermissionButtonStateBusy(true, getString(R.string.koda_waiting));
        mStatusText.setText(getString(R.string.koda_shizuku_requesting_permission));
        Shizuku.removeRequestPermissionResultListener(mShizukuPermissionResultListener);
        Shizuku.addRequestPermissionResultListener(mShizukuPermissionResultListener);
        Toast.makeText(this, getString(R.string.koda_shizuku_requesting_permission), Toast.LENGTH_SHORT).show();
        Shizuku.requestPermission(SHIZUKU_PERMISSION_REQUEST_CODE);
    }

    private void runShizukuGatewayPrecheck() {
        if (mKodaService == null) {
            Logger.logWarn(LOG_TAG, "Shizuku precheck skipped: KodaService not connected");
            Toast.makeText(this, getString(R.string.koda_shizuku_precheck_service_not_connected), Toast.LENGTH_SHORT).show();
            runShizukuPermissionButtonStateBusy(false, null);
            mStatusText.setText(getString(R.string.koda_shizuku_precheck_service_not_connected));
            return;
        }

        Logger.logInfo(LOG_TAG, "Running Shizuku precheck command: " + OPENCLAUDE_GATEWAY_PRECHECK_COMMAND);
        runShizukuPermissionButtonStateBusy(true, getString(R.string.koda_checking));
        mStatusText.setText(getString(R.string.koda_shizuku_precheck_running));

        mKodaService.executeCommand(OPENCLAUDE_GATEWAY_PRECHECK_COMMAND, result -> {
            runOnUiThread(() -> {
                runShizukuPermissionButtonStateBusy(false, null);
                if (result == null) {
                    Logger.logWarn(LOG_TAG, "Shizuku precheck returned null result");
                    mStatusText.setText(getString(R.string.koda_shizuku_precheck_no_result));
                    Toast.makeText(AutomationPanelActivity.this, getString(R.string.koda_shizuku_precheck_no_result), Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                if (result.success) {
                    mStatusText.setText(getString(R.string.koda_shizuku_precheck_passed));
                    Toast.makeText(AutomationPanelActivity.this, getString(R.string.koda_shizuku_precheck_passed), Toast.LENGTH_SHORT).show();
                } else {
                    String reason = TextUtils.isEmpty(result.stderr) ? result.stdout : result.stderr;
                    String shownReason = TextUtils.isEmpty(reason) ? getString(R.string.koda_unknown) : reason;
                    mStatusText.setText(
                            getString(R.string.koda_shizuku_precheck_failed, shownReason)
                    );
                    Toast.makeText(
                            AutomationPanelActivity.this,
                            getString(R.string.koda_shizuku_precheck_failed, shownReason),
    ...
    private void refreshShizukuPermissionState() {
        ...
        try {
            ...
            if (permission == PackageManager.PERMISSION_GRANTED) {
                mStatusText.setText(getString(R.string.koda_shizuku_permission_granted));
            } else {
                mStatusText.setText(getString(R.string.koda_shizuku_permission_not_granted));
            }
        } catch (Throwable tr) {
            mStatusText.setText(getString(R.string.koda_shizuku_status_unavailable));
    ...
    private void checkShizukuBridgeStatus() {
        ...
        if (TextUtils.isEmpty(mShizukuBridgeStatusText.getText())) {
            mShizukuBridgeStatusText.setText(getString(R.string.koda_shizuku_bridge_status_checking));
        }
        ...
        new Thread(() -> {
            ...
            if (TextUtils.isEmpty(token)) {
                result = new ServiceStateResult(false, getString(R.string.koda_shizuku_bridge_status_not_configured));
            } else {
            ...
            runOnUiThread(() -> {
                ...
                if (result.running) {
                    mShizukuBridgeStatusText.setText(getString(R.string.koda_shizuku_bridge_status_running));
                } else if (TextUtils.isEmpty(result.message)) {
                    mShizukuBridgeStatusText.setText(getString(R.string.koda_shizuku_bridge_status_stopped));
                } else {
                    mShizukuBridgeStatusText.setText(
                        getString(R.string.koda_shizuku_bridge_status_stopped_with_reason, result.message)
                    );
                }
            });
        }).start();
    ...
    private void checkU2ServiceStatus() {
        ...
        if (TextUtils.isEmpty(mU2StatusText.getText())) {
            mU2StatusText.setText(getString(R.string.koda_u2_service_status_checking));
        }
        executeShizukuShellCommand(buildCheckU2StatusCommand(), result -> {
            ...
            if (stateResult.running) {
                mU2StatusText.setText(getString(R.string.koda_u2_service_status_running));
                ...
                mU2StatusText.setText(getString(R.string.koda_u2_service_status_stopped));
                setU2ServiceButtonsState(true, false);
            } else {
                mU2StatusText.setText(
                    getString(R.string.koda_u2_service_status_stopped_with_reason, stateResult.message)
                );
                setU2ServiceButtonsState(true, false);
            }
        });
    ...
    private void startU2Service() {
        ...
        setU2ServiceButtonsState(false, false);
        mU2StatusText.setText(getString(R.string.koda_u2_service_status_checking));
        Toast.makeText(this, getString(R.string.koda_u2_service_starting), Toast.LENGTH_SHORT).show();
        showU2StartProgressDialog();
        setU2StartProgressMessage(R.string.koda_u2_service_prepare_step_prepare_env);
        ...
        new Thread(() -> {
            ...
            if (!TextUtils.isEmpty(copyMessage)) {
                ...
                runOnUiThread(() -> {
                    Toast.makeText(this, getString(R.string.koda_u2_service_start_failed) + ": " + copyMessage,
                            Toast.LENGTH_LONG).show();
                    Logger.logWarn(LOG_TAG, "Prepare u2 jar failed: " + copyMessage);
                    showU2StartProgressFailure(
                        "copy_u2_failed",
                        getString(R.string.koda_u2_service_start_failed) + ": " + copyMessage
                    );
                    checkU2ServiceStatus();
                });
                return;
            ...
            mKodaService.runEnsureSharpInstalled(sharpResult -> {
                if (!isUiAvailable()) return;

                // Check if u2automator is already installed
                setU2StatusText(getString(R.string.koda_u2_service_status_checking));
                mKodaService.executeU2SetupCommand(buildCheckU2AutomatorInstalledCommand(), checkResult -> {
                    if (!isUiAvailable()) return;
    ...
    private void startU2Jar() {
        setU2StatusText(getString(R.string.koda_u2_service_prepare_step_start_u2));
        setU2StartProgressMessage(R.string.koda_u2_service_prepare_step_start_u2);
        ...
        executeShizukuShellCommand(buildStartU2Command(), step4 -> {
            ...
            runOnUiThread(() -> {
                Toast.makeText(this,
                        getString(R.string.koda_u2_service_start_command_sent),
    ...
    private void runU2SetupSteps() {
        ...
        setU2StatusText(getString(R.string.koda_u2_service_prepare_step_clean_sources));
        setU2StartProgressMessage(R.string.koda_u2_service_prepare_step_clean_sources);
        setU2StartProgressStep(0);
        mKodaService.executeU2SetupCommand(buildTrimAptSourcesForU2Command(), step1 -> {
            if (!isUiAvailable()) return;
            ...
            setU2StatusText(getString(R.string.koda_u2_service_prepare_step_prepare_env));
            setU2StartProgressMessage(R.string.koda_u2_service_prepare_step_prepare_env);
            setU2StartProgressStep(1);
            mKodaService.executeU2SetupCommand(buildReinstallDependenciesCommand(), 180, step2 -> {
                if (!isUiAvailable()) return;
                ...
                setU2StatusText(getString(R.string.koda_u2_service_prepare_step_verify));
                setU2StartProgressMessage(R.string.koda_u2_service_prepare_step_verify);
                setU2StartProgressStep(2);
                mKodaService.executeU2SetupCommand(buildInstallU2AutomatorCommand(), 600, step3 -> {
                    if (!isUiAvailable()) return;
    ...
    private void handleU2StartStepFailure(String stepName,
            @Nullable KodaService.CommandResult result) {
        String detail = resolveCommandFailure(result);
        String reason = mapU2StartFailureReason(stepName);
        mU2SetupInProgress = false;
        runOnUiThread(() -> {
            if (!isUiAvailable()) return;
            String message = getString(R.string.koda_u2_service_start_failed) + ": " + detail;
    ...
    private void installAdbKeyboardIme(Runnable onComplete) {
        setU2StatusText(getString(R.string.koda_u2_service_prepare_step_install_ime));
        setU2StartProgressMessage(R.string.koda_u2_service_prepare_step_install_ime);
        setU2StartProgressStep(3);
        ...
        executeShizukuShellCommand("pm list packages " + U2_IME_PACKAGE, checkResult -> {
            ...
            new Thread(() -> {
                ...
                if (error != null) {
                    ...
                    runOnUiThread(() -> {
                        if (!isUiAvailable()) return;
                        String message = getString(R.string.koda_u2_service_start_failed) + ": " + error;
                        Toast.makeText(this, message, Toast.LENGTH_LONG).show();
                        setU2StatusText(message);
    ...
    private void showU2StartProgressDialog() {
        ...
        mU2StartProgressDialog = StepProgressDialog.create(
            this,
            R.string.koda_u2_service_starting,
            Arrays.asList(
                getString(R.string.koda_u2_service_prepare_step_clean_sources),
                getString(R.string.koda_u2_service_prepare_step_prepare_env),
                getString(R.string.koda_u2_service_prepare_step_verify),
                getString(R.string.koda_u2_service_prepare_step_install_ime),
                getString(R.string.koda_u2_service_prepare_step_start_u2)
            ),
            getString(R.string.koda_u2_service_status_checking)
        );
    ...
    private void completeU2StartProgress() {
        ...
        mU2StartProgressDialog.complete(getString(R.string.koda_u2_service_start_command_sent));
    ...
    private String buildTrimAptSourcesForU2Command() {
        return "export PREFIX=\"" + TermuxConstants.TERMUX_PREFIX_DIR_PATH + "\"\n"
            ...
            + "  echo \"No Koda apt source found\" >&2\n"
            + "  exit 1\n"
            + "fi\n"
    ...
    private void stopU2Service() {
        ...
        setU2ServiceButtonsState(false, false);
        mU2StatusText.setText(getString(R.string.koda_u2_service_status_checking));
        Toast.makeText(this, getString(R.string.koda_u2_service_stopping), Toast.LENGTH_SHORT).show();
        ...
        executeShizukuShellCommand(buildStopU2Command(), result -> {
            ...
            runOnUiThread(() -> {
                if (!isUiAvailable() || mKodaService == null) {
                    return;
                }
                ...
                if (result == null) {
                    ...
                    Toast.makeText(
                            this,
                            getString(R.string.koda_u2_service_stop_failed),
                            Toast.LENGTH_SHORT
                    ).show();
                ...
                if (result.success) {
                    ...
                    Toast.makeText(
                            this,
                            getString(R.string.koda_u2_service_stop_command_sent),
                            Toast.LENGTH_SHORT
                    ).show();
                    mHandler.postDelayed(this::checkU2ServiceStatus, SERVICE_REFRESH_DELAY_MS);
                } else {
                    String shownReason = TextUtils.isEmpty(result.stderr) ? getString(R.string.koda_unknown) : result.stderr;
                    Toast.makeText(
                            this,
                            getString(R.string.koda_u2_service_stop_failed) + ": " + shownReason,
                            Toast.LENGTH_LONG
                    ).show();
    ...
    private void executeShizukuShellCommand(String command, KodaService.CommandCallback callback) {
    ...
    private void executeShizukuShellCommand(
        String command,
        long timeoutMs,
        KodaService.CommandCallback callback
    ) {
        new Thread(() -> {
            if (mShizukuShellExecutor == null) {
                runOnUiThread(() -> {
                    if (callback != null) {
                        callback.onResult(new KodaService.CommandResult(false, "", "Shizuku shell executor not initialized", -1));
                    }
                });
                return;
            }
            ...
            if (!mShizukuShellExecutor.isBound()) {
                if (!mShizukuShellExecutor.waitForConnection(SHIZUKU_SHELL_CONNECT_TIMEOUT_MS)) {
                    runOnUiThread(() -> {
                        if (callback != null) {
                            callback.onResult(new KodaService.CommandResult(false, "", "Shizuku shell unavailable", -1));
                        }
                    });
                    return;
                }
    ...
    private String resolveCommandFailure(@Nullable KodaService.CommandResult result) {
        if (result == null) {
            return getString(R.string.koda_unknown);
        }
