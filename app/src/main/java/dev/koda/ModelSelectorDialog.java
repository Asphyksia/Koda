 ...
 * Uses cached OpenClaude model list when possible and fallbacks to static catalog.
 */
public class ModelSelectorDialog extends Dialog {

    private static final String LOG_TAG = "ModelSelectorDialog";
    private static final String PREFS_NAME = "openclaude_model_cache_v1";
    private static final String KEY_CACHE_PREFS_NAME = "openclaude_model_key_cache_v1";
    private static final String STATIC_MODELS_ASSET = "openclaude-models-all.keys";
    ...
    private static final String CUSTOM_PROVIDER_ID = KodaConfig.CUSTOM_PROVIDER_ID;
    private static final int CUSTOM_PROVIDER_DISPLAY_NAME_RES = R.string.koda_custom_provider;
    private static final int PROVIDER_SECTION_CONFIGURED_RES = R.string.koda_provider_section_configured;
    private static final int PROVIDER_SECTION_UNCONFIGURED_RES = R.string.koda_provider_section_unconfigured;
    private static final int PROVIDER_STATUS_CONFIGURED_RES = R.string.koda_provider_status_configured;
    private static final int PROVIDER_STATUS_UNCONFIGURED_RES = R.string.koda_provider_status_unconfigured;

    // Cached in-memory for the currently active OpenClaude version.
    private static List<ModelInfo> sCachedAllModels;
    private static String sCachedVersion;

    private final KodaService mService;
    ...
    public ModelSelectorDialog(@NonNull Context context, KodaService service) {
        this(context, service, false);
    }

    public ModelSelectorDialog(@NonNull Context context, KodaService service, boolean promptForApiKey) {
        super(context);
    ...
    private void loadModels(boolean forceRefresh) {
        ...
        String openclaudeVersion = KodaService.getOpenclaudeVersion();
        if (TextUtils.isEmpty(openclaudeVersion)) {
            openclaudeVersion = "unknown";
        }
        String normalizedVersion = normalizeCacheKey(openclaudeVersion);
        final String versionForLog = openclaudeVersion;
        ...
        if (mService == null) {
            ...
            if (!models.isEmpty()) {
                showModelsFromList(getDialogText(R.string.koda_fallback_to_bundled_catalog), models);
                return;
            }
            showError(getDialogText(R.string.koda_failed_to_load_model_catalog));
            return;
        }

        mService.executeCommand(OpenclaudeModelListUtils.buildPreferredModelListCommand(true), result -> {
            if (!result.success) {
                Logger.logError(LOG_TAG, "Failed to load models from OpenClaude: exit " + result.exitCode);
                List<ModelInfo> cached = loadCachedModels(normalizedVersion);
                if (!cached.isEmpty()) {
                    sCachedVersion = normalizedVersion;
                    sCachedAllModels = cached;
                ...
                if (!fallback.isEmpty()) {
                    showModelsFromList(
                        getDialogText(R.string.koda_failed_to_load_from_openclaude_using_bundled_catalog),
                        fallback
                    );
                    return;
                }
            ...
            if (models.isEmpty()) {
                ...
                if (!fallback.isEmpty()) {
                    showModelsFromList(
                        getDialogText(R.string.koda_failed_to_parse_openclaude_output_using_bundled_catalog),
                        fallback
                    );
                    return;
                }
                showError(getDialogText(R.string.koda_no_model_list_available));
                return;
            }
            ...
            Logger.logInfo(LOG_TAG, "Loaded " + models.size() + " models for OpenClaude v" + versionForLog);
            showModelsFromList(
                getDialogText(R.string.koda_loaded_models_from_openclaude, models.size()),
                models
            );
        });
    }
        ...
        if (models.isEmpty()) {
            showError(
                TextUtils.isEmpty(sourceMessage)
                    ? getDialogText(R.string.koda_no_model_list_available)
                    : sourceMessage
            );
            return;
        }
    ...
    private void showProviderCredentialPrompt(String provider) {
        ...
        String savedBaseUrl = KodaConfig.getBaseUrl(provider);
        ...
        boolean hasExistingKey = KodaConfig.hasApiKey(providerForStorage);
        String currentProviderKey = KodaConfig.getApiKey(providerForStorage);
        ...
        noteText.setText(isCustomProvider(provider)
            ? getDialogText(R.string.koda_enter_provider_credentials_custom)
            : getDialogText(R.string.koda_enter_api_key_for_provider));
        ...
        apiKeyInput.setHint(
            hasExistingKey
                ? getDialogText(R.string.koda_leave_empty_to_keep_current_key)
                : getDialogText(R.string.koda_enter_api_key)
        );
        apiKeyInput.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
        apiKeyInput.setTextColor(getContext().getColor(R.color.koda_on_background));
        apiKeyInput.setText("");
        ...
        Runnable updateConfirmButton = () -> {
            ...
            confirmButton.setText(inDeleteMode[0]
                ? getDialogText(R.string.koda_confirm)
                : getDialogText(useCustomEndpoint ? R.string.koda_fetch_models : R.string.koda_continue));
        };
        ...
        Runnable submitProviderCredentials = () -> {
            ...
            if (useCustomEndpoint) {
                ...
                if (TextUtils.isEmpty(effectiveBaseUrl)) {
                    baseUrlInput.setError(getDialogText(R.string.koda_base_url_required_for_provider));
                    return;
                }
            ...
            if (TextUtils.isEmpty(resolvedProvider) && !TextUtils.isEmpty(requestedProviderFromInput)) {
                ...
                if (TextUtils.isEmpty(resolvedProvider)) {
                    if (providerIdInput != null) {
                        providerIdInput.setError(getDialogText(R.string.koda_provider_id_required));
                    }
                    return;
                }
                ...
                if (isCustomProvider(provider) && isDuplicateProviderId(resolvedProvider)) {
                    if (providerIdInput != null) {
                        providerIdInput.setError(getDialogText(R.string.koda_provider_name_exists));
                    }
                    return;
                }
            ...
            if (TextUtils.isEmpty(effectiveApiKey)) {
                String cachedKeyForResolvedProvider = KodaConfig.getApiKey(resolvedProvider);
                if (!TextUtils.isEmpty(cachedKeyForResolvedProvider)) {
                    effectiveApiKey = cachedKeyForResolvedProvider;
                }
            }
            if (TextUtils.isEmpty(effectiveApiKey)) {
                apiKeyInput.setError(getDialogText(R.string.koda_api_key_required_for_provider));
                return;
            }
    ...
    private void loadCustomModels(String provider, String baseUrl, String apiKey) {
        if (TextUtils.isEmpty(provider) || TextUtils.isEmpty(baseUrl) || TextUtils.isEmpty(apiKey)) {
            showError(getDialogText(R.string.koda_custom_provider_base_url_and_api_key_required));
            return;
        }

        showLoading();
        if (mStatusText != null) {
            mStatusText.setText(getDialogText(R.string.koda_loading_models_from_custom_url));
        }
        ...
        new Thread(() -> {
            ...
            new Handler(Looper.getMainLooper()).post(() -> {
                ...
                if (models == null || models.isEmpty()) {
                    showError(getDialogText(R.string.koda_no_models_returned_by_custom_provider));
                    clearPendingCredentials();
                    return;
                }
    ...
    private void renderCachedKeys(String provider, EditText apiKeyInput, LinearLayout container, TextView titleText, Runnable onCacheUpdated) {
        ...
        if (titleText != null) {
            ...
            titleText.setText(getDialogText(R.string.koda_cached_keys));
        }
        ...
        for (String key : cachedKeys) {
            ...
            row.setBackgroundResource(R.drawable.koda_input_bg);
            row.setPadding(
            ...
            keyText.setTextColor(getContext().getColor(R.color.koda_on_background));
            ...
            useAction.setText(getDialogText(R.string.koda_use));
            useAction.setTextSize(12f);
            useAction.setTextColor(getContext().getColor(R.color.koda_accent));
            ...
            deleteAction.setContentDescription(getDialogText(R.string.koda_delete_cached_key));
    ...
    private List<String> loadCachedApiKeys(String provider) {
        ...
        try {
            ...
            String storedProviderKey = KodaConfig.getApiKey(provider);
    ...
    private void showProviderSelection() {
        ...
        if (mPromptForApiKey) {
            ...
            customProviderItem.statusText = getDialogText(R.string.koda_add_custom_provider);

            List<String> customProviders = KodaConfig.getConfiguredCustomProviders();
        ...
        if (mCurrentItems.isEmpty()) {
            mCurrentItems.add(createSectionHeader(getDialogText(R.string.koda_no_provider_available)));
        }

        mSearchBox.setText("");
        mSearchBox.setHint(getDialogText(R.string.koda_search_provider));
        mBackButton.setVisibility(View.GONE);
        mTitleText.setText(getDialogText(R.string.koda_select_provider));

        mAdapter.updateList(mCurrentItems);
        if (mCurrentItems.isEmpty()) {
            showError(getDialogText(R.string.koda_no_provider_available));
            return;
        }
        showList();
    }
    ...
    private ModelInfo createProviderListItem(String provider, String statusText, boolean asHeader) {
        if (TextUtils.isEmpty(provider)) {
            return createSectionHeader(getDialogText(R.string.koda_no_provider_available));
        }
    ...
    private boolean isDuplicateProviderId(String providerId) {
        ...
        for (String configuredProvider : KodaConfig.getConfiguredCustomProviders()) {
    ...
    private void showModelSelection(String provider, List<ModelInfo> modelsForProvider) {
        ...
        mSearchBox.setText("");
        mSearchBox.setHint(getDialogText(R.string.koda_search_model));
        mBackButton.setVisibility(View.VISIBLE);
        String providerName = isCustomProvider(provider) ? getDialogText(CUSTOM_PROVIDER_DISPLAY_NAME_RES) : provider;
        mTitleText.setText(getDialogText(R.string.koda_provider_models, providerName));

        mAdapter.updateList(mCurrentItems);
        if (mCurrentItems.isEmpty()) {
            showError(getDialogText(R.string.koda_no_models_available_for_provider, providerName));
            return;
        }
        showList();
    }
    ...
    private void showLoading() {
        ...
        mStatusText.setText(getDialogText(R.string.koda_loading_models));
    }
