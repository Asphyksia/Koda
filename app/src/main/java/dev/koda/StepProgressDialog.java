...
public final class StepProgressDialog {
    ...
    private StepProgressDialog(
        ...
        mContext = context;
        View dialogView = LayoutInflater.from(context).inflate(R.layout.dialog_openclaude_update, null);
        TextView[] stepTexts = {
        ...
        if (mStatusText != null) {
            mStatusText.setText(TextUtils.isEmpty(initialStatus)
                    ? context.getString(R.string.koda_may_take_a_few_minutes)
                    : initialStatus);
        }
        ...
        mDialog = KodaDialogStyler.createBuilder(context)
            .setTitle(titleRes)
            .setView(dialogView)
            .setCancelable(false)
            .create();
    ...
    public void show() {
        mDialog.show();
        KodaDialogStyler.applyTransparentCardWindow(mDialog);
    }
    ...
    public void setStatus(@Nullable String message) {
        if (mStatusText != null) {
            mStatusText.setText(TextUtils.isEmpty(message)
                    ? mContext.getString(R.string.koda_may_take_a_few_minutes)
                    : message);
        }
    }
    ...
    public void showError(@Nullable String errorMessage, @Nullable Runnable onClose) {
        ...
        if (mCloseButton != null) {
            mCloseButton.setText(mContext.getString(R.string.koda_close));
            ...
            mDialog.setButton(AlertDialog.BUTTON_NEGATIVE, mContext.getString(R.string.koda_close),
                (dialog, which) -> {
                    if (onClose != null) {
                        onClose.run();
                    }
