...
import dev.koda.DashboardActivity;
import com.termux.R;
...
public class ShizukuBridgeService extends Service {
    ...
    private void writeBridgeConfig() {
        ...
        try {
            ...
            File openclaudeDir = new File(home, ".openclaude");
            if (!openclaudeDir.exists() && !openclaudeDir.mkdirs()) {
                Logger.logWarn(LOG_TAG, "Unable to create openclaude dir");
                return;
            }

            File config = new File(openclaudeDir, "shizuku-bridge.json");
    ...
    private void deleteBridgeConfig() {
        try {
            File config = new File(TermuxConstants.TERMUX_HOME_DIR_PATH + "/.openclaude/shizuku-bridge.json");
            if (config.exists()) {
                config.delete();
            }
    ...
    private Notification buildNotification(String text) {
        ...
        return new NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Koda Shizuku Bridge")
            .setContentText(text)
