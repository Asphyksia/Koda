 ...
 * 2. If OpenClaude not installed/configured -> SetupActivity (agent -> install -> auth)
 * 3. If channel not configured -> SetupActivity (channel setup)
 * 4. All ready -> DashboardActivity
 */
public class KodaLauncherActivity extends Activity {

    private static final String LOG_TAG = "KodaLauncherActivity";
    ...
    private static final String PREFS_NAME = "koda_launcher";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_koda_launcher);
        ...
        mUpdateManagementDisabled = BundledOpenclaudeUtils.shouldDisableUpdateManagement(this);

        mContinueClickedPersisted = getSharedPreferences(PREFS_NAME, MODE_PRIVATE)
            .getBoolean(PREF_ONBOARDING_CONTINUE, false);

        // Upgrade migration: clean deprecated keys from existing OpenClaude config.
    ...
    private void showWelcomePhase() {
        ...
        AnalyticsManager.logScreen(this, "launcher_welcome", "KodaLauncherActivity");
    }

    private void showLoadingPhase() {
        mWelcomeContainer.setVisibility(View.GONE);
        mLoadingContainer.setVisibility(View.VISIBLE);
        AnalyticsManager.logScreen(this, "launcher_loading", "KodaLauncherActivity");
    }
    ...
    private void updatePermissionStatus() {
        ...
        if (notifGranted) {
            ...
            mNotificationButton.setText(R.string.koda_enabled);
        } else {
            mNotificationStatus.setVisibility(View.GONE);
            mNotificationButton.setEnabled(true);
            mNotificationButton.setText(R.string.koda_allow);
        }
        ...
        if (batteryExempt) {
            ...
            mBatteryButton.setText(R.string.koda_granted);
        } else {
            mBatteryStatus.setVisibility(View.GONE);
            mBatteryButton.setEnabled(true);
            mBatteryButton.setText(R.string.koda_allow);
        }

        // Background hint status (informational)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            if (backgroundStatus == ConnectivityManager.RESTRICT_BACKGROUND_STATUS_ENABLED) {
                mBackgroundHintText.setText(R.string.koda_background_data_restricted);
            } else if (backgroundStatus == ConnectivityManager.RESTRICT_BACKGROUND_STATUS_WHITELISTED) {
                mBackgroundHintText.setText(R.string.koda_background_data_allowed);
            } else {
                mBackgroundHintText.setText(R.string.koda_background_data_hint);
            }
        }
    ...
    private void checkAndRoute() {
        ...
        if (!KodaService.isBootstrapInstalled()) {
            ...
            mStatusText.setText(R.string.koda_setting_up_environment);

            TermuxInstaller.setupBootstrapIfNeeded(this, this::checkAndRoute);
            return;
        }

        // Check 2: OpenClaude installed?
        if (!KodaService.isOpenclaudeInstalled()) {
            Logger.logInfo(LOG_TAG, "OpenClaude not installed, routing to agent selection");
            mStatusText.setText(R.string.koda_setup_required);
        ...
        // Check 3: OpenClaude configured (API key)?
        if (!KodaService.isOpenclaudeConfigured()) {
            Logger.logInfo(LOG_TAG, "OpenClaude not configured, routing to agent-first setup");
            mStatusText.setText(R.string.koda_setup_required);
        ...
        if (!hasChannelConfigured()) {
            ...
            mStatusText.setText(R.string.koda_channel_setup_required);
        ...
        mStatusText.setText(R.string.koda_starting_status);
