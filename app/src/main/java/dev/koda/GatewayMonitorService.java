...
/**
 * Foreground service that monitors and keeps the OpenClaude gateway alive.
 *
 * Features:
...
public class GatewayMonitorService extends Service {
    ...
    private static final String APP_UPDATE_PREFS_NAME = "koda_update";
    ...
    private static final String UPDATE_NOTIFICATION_CHANNEL_ID = "koda_updates";
    ...
    private boolean mKodaServiceBound = false;
    ...
    /**
     * Service connection for binding to KodaService
     */
    private ServiceConnection mKodaServiceConnection = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
            KodaService.LocalBinder binder = (KodaService.LocalBinder) service;
            mKodaService = binder.getService();
            mKodaServiceBound = true;
            Logger.logInfo(LOG_TAG, "Bound to KodaService");
        ...
        public void onServiceDisconnected(ComponentName name) {
            mKodaService = null;
            mKodaServiceBound = false;
            Logger.logInfo(LOG_TAG, "Disconnected from KodaService");
    ...
    private void scheduleRebind() {
        ...
        mHandler.postDelayed(() -> {
            ...
            if (mKodaServiceBound) return;
            try {
                Intent intent = new Intent(this, KodaService.class);
                // Ensure service is started so the binding has a live target.
                startService(intent);
                bindService(intent, mKodaServiceConnection, Context.BIND_AUTO_CREATE);
                Logger.logInfo(LOG_TAG, "Rebinding to KodaService");
            } catch (Exception e) {
                Logger.logError(LOG_TAG, "Failed to rebind KodaService: " + e.getMessage());
                scheduleRebind();
            }
        }, 2000);
    }
    ...
    public void onCreate() {
        ...
        // Start + bind to KodaService for command execution. Binding alone can be fragile
        ...
        try {
            ...
            if (wifiManager != null) {
                mWifiLock = wifiManager.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "Koda::GatewayWifi");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        Notification notification = buildNotification("Koda is running");
        startForeground(NOTIFICATION_ID, notification);

        // Monitoring will start automatically when KodaService is bound
    ...
    public void onDestroy() {
        ...
        // Unbind from KodaService
        if (mKodaServiceBound) {
            try {
                unbindService(mKodaServiceConnection);
                Logger.logInfo(LOG_TAG, "Unbound from KodaService");
    ...
    private void postAppUpdateNotification(String latestVersion, String downloadUrl) {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, UPDATE_NOTIFICATION_CHANNEL_ID)
            .setContentTitle(getString(R.string.koda_update_title))
            .setContentText(getString(R.string.koda_new_version_detected, latestVersion))
            .setStyle(new NotificationCompat.BigTextStyle().bigText(getString(R.string.koda_new_version_detected_detail, latestVersion)))
    ...
    private void createNotificationChannels() {
        ...
        NotificationChannel updateChannel = new NotificationChannel(
            ...
            getString(R.string.koda_update_channel_name),
            NotificationManager.IMPORTANCE_DEFAULT
        );
        updateChannel.setDescription(getString(R.string.koda_update_channel_description));
    ...
    private void checkAndRestartGateway() {
        ...
        if (!mKodaServiceBound || mKodaService == null) {
            Logger.logDebug(LOG_TAG, "KodaService not bound yet, scheduling rebind");
            scheduleRebind();
            return;
        }

        // Skip monitoring during OpenClaude update to avoid restarting mid-install.
        if (mKodaService.isUpdateInProgress()) {
            Logger.logInfo(LOG_TAG, "OpenClaude update in progress, skipping gateway check");
    ...
    private void restartGateway() {
        // Only proceed if service is bound
        if (!mKodaServiceBound || mKodaService == null) {
            Logger.logError(LOG_TAG, "Cannot restart: KodaService not bound");
            return;
        }
