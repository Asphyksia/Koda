...
public class AuthFragment extends Fragment implements SetupActivity.StepFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_koda_auth, container, false);
    ...
    private void openModelSelector() {
        ...
        dialog.show((provider, model, apiKey, baseUrl, availableModels) -> {
            ...
            mSelectedProviderBaseUrl = normalizeCustomBaseUrl(!TextUtils.isEmpty(baseUrl) ? baseUrl : KodaConfig.getBaseUrl(provider));
            ...
            mKeyLabel.setText(getString(R.string.koda_api_key));
    ...
    private ProviderInfo findProviderById(String providerId) {
        ...
        return new ProviderInfo(
            providerId,
            providerId,
            getString(R.string.koda_api_key),
    ...
    private void verifyAndContinue() {
        if (mSelectedModel == null || mSelectedProvider == null) {
            ...
            showStatus(getString(R.string.koda_select_model_first), false);
            return;
        }
        ...
        if (isCustomProvider()) {
            ...
            if (!TextUtils.equals(baseUrl, selectedBaseUrl)) {
                ...
                showStatus(getString(R.string.koda_base_url_changed_reselect_provider_model), false);
                return;
            }

            if (TextUtils.isEmpty(baseUrl)) {
                AnalyticsManager.logEvent(requireContext(), "auth_verify_failed", "reason", "missing_base_url");
                showStatus(getString(R.string.koda_enter_custom_base_url), false);
                return;
            }
        }
        if (TextUtils.isEmpty(credential)) {
            AnalyticsManager.logEvent(requireContext(), "auth_verify_failed", "reason", "missing_api_key");
            showStatus(getString(R.string.koda_enter_api_key), false);
            return;
        }

        if (credential.length() < 8) {
            AnalyticsManager.logEvent(requireContext(), "auth_verify_failed", "reason", "invalid_api_key_format");
            showStatus(getString(R.string.koda_invalid_api_key_format), false);
            return;
        }

        mVerifyButton.setEnabled(false);
        mVerifyButton.setText(getString(R.string.koda_verifying));
        showStatus(getString(R.string.koda_verifying_credentials), true);
    ...
    private void saveCredentials(String credential, String baseUrl) {
        ...
        if (isCustomProvider && (mCurrentCustomModels == null || mCurrentCustomModels.isEmpty())) {
            ...
            showStatus(getString(R.string.koda_no_custom_model_list_reselect), false);
            return;
        }

        boolean saved = KodaConfig.setActiveProvider(
            providerId,
            modelToUse,
            credential,
        ...
        if (saved) {
            ...
            showStatus(getString(R.string.koda_connected_with_model, fullModel), true);
            ...
            showStatus(getString(R.string.koda_error_write_config), false);
            resetVerifyButton();
        }
    }
    ...
    private void resetVerifyButton() {
        ...
        mVerifyButton.setText(getString(R.string.koda_verify_continue));
    }
}
