...
/**
 * Static utility for OpenClaude version parsing, normalization, and comparison.
 * Shared between AgentSelectionFragment and DashboardActivity.
 */
public final class OpenclaudeVersionUtils {

    public static final String VERSION_PREFIX = "openclaude@";
    ...
    public static final String MODEL_LIST_COMMAND = "openclaude models list --all --plain";
    ...
    public static final int OPENCLAUDE_MIN_OLD_SPACE_MB = 1536;
    public static final int OPENCLAUDE_MAX_OLD_SPACE_MB = 4096;
    ...
    private static final String OPENCLAUDE_DEFAULT_OLD_SPACE_ENV =
        "KODA_OPENCLAUDE_DEFAULT_MAX_OLD_SPACE_MB";
    private static final int OPENCLAUDE_OLD_SPACE_UP_TO_8_GB_MB = 2048;
    private static final int OPENCLAUDE_OLD_SPACE_10_GB_MB = 2560;
    private static final int OPENCLAUDE_OLD_SPACE_12_GB_MB = 3072;

    public interface VersionListCallback {
        void onResult(List<String> versions, String errorMessage);
    }

    private OpenclaudeVersionUtils() {}
    ...
    public static String buildVersionsCommand() {
        return buildVersionEnvironmentPrepCommand() + buildNpmAwareCommand("npm view openclaude versions --json");
    }

    public static String buildLatestVersionCommand() {
        return "set -o pipefail; " + buildNpmAwareCommand(
            "npm view openclaude version 2>/dev/null | tail -1 | tr -d '[:space:]'");
    }

    public static String buildNpmInstallCommand(String packageSpec) {
        String safePackage = shellQuoteSingle(TextUtils.isEmpty(packageSpec) ? "openclaude@latest" : packageSpec);
    ...
    public static int recommendOpenclaudeOldSpaceMb(long totalRamMb) {
        if (totalRamMb <= RAM_4_GB_MB) {
            return OPENCLAUDE_MIN_OLD_SPACE_MB;
        }
        if (totalRamMb <= RAM_8_GB_MB) {
            return OPENCLAUDE_OLD_SPACE_UP_TO_8_GB_MB;
        }
        if (totalRamMb <= RAM_10_GB_MB) {
            return OPENCLAUDE_OLD_SPACE_10_GB_MB;
        }
        if (totalRamMb <= RAM_12_GB_MB) {
            return OPENCLAUDE_OLD_SPACE_12_GB_MB;
        }
        return OPENCLAUDE_MAX_OLD_SPACE_MB;
    }

    public static String buildOpenclaudeNodeOptions(String existingOptions, long totalRamMb) {
        ...
        if (!containsNodeOption(options, "--max-old-space-size=")) {
            options = appendNodeOption(options,
                "--max-old-space-size=" + recommendOpenclaudeOldSpaceMb(totalRamMb));
        }
    ...
    public static String buildNodeOptionsExportCommand(int oldSpaceMb) {
        return "export " + OPENCLAUDE_DEFAULT_OLD_SPACE_ENV + "=" + oldSpaceMb + "\n"
            + buildNodeOptionsExportCommand();
    }

    public static String buildNodeOptionsExportCommand() {
        return "koda_set_openclaude_node_options() {\n"
            + "  node_options=\"${NODE_OPTIONS:-}\"\n"
            + "  old_space_mb=\"${KODA_OPENCLAUDE_MAX_OLD_SPACE_MB:-${"
            + OPENCLAUDE_DEFAULT_OLD_SPACE_ENV + ":-}}\"\n"
            ...
            + "      old_space_mb=" + OPENCLAUDE_MIN_OLD_SPACE_MB + "\n"
            + "    else\n"
            + "      mem_total_mb=$((mem_total_kb / 1024))\n"
            + "      if [ \"$mem_total_mb\" -le " + RAM_4_GB_MB + " ]; then\n"
            + "        old_space_mb=" + OPENCLAUDE_MIN_OLD_SPACE_MB + "\n"
            + "      elif [ \"$mem_total_mb\" -le " + RAM_8_GB_MB + " ]; then\n"
            + "        old_space_mb=" + OPENCLAUDE_OLD_SPACE_UP_TO_8_GB_MB + "\n"
            + "      elif [ \"$mem_total_mb\" -le " + RAM_10_GB_MB + " ]; then\n"
            + "        old_space_mb=" + OPENCLAUDE_OLD_SPACE_10_GB_MB + "\n"
            + "      elif [ \"$mem_total_mb\" -le " + RAM_12_GB_MB + " ]; then\n"
            + "        old_space_mb=" + OPENCLAUDE_OLD_SPACE_12_GB_MB + "\n"
            + "      else\n"
            + "        old_space_mb=" + OPENCLAUDE_MAX_OLD_SPACE_MB + "\n"
            + "      fi\n"
            + "    fi\n"
            + "  fi\n"
            ...
            + "}\n"
            + "koda_set_openclaude_node_options\n";
    }

    public static String buildOpenclaudeWrapperBody(String packageRootExpr, String nodePathExpr, int oldSpaceMb) {
        return "PACKAGE_ROOT=\"" + packageRootExpr + "\"\n"
            ...
            + "  \"$PACKAGE_ROOT/bin/openclaude.js\" \\\n"
            ...
            + "done\n"
            + "if [ -z \"$ENTRY\" ]; then\n"
            + "  echo \"openclaude entrypoint not found under $PACKAGE_ROOT\" >&2\n"
            + "  exit 127\n"
            + "fi\n"
    ...
    public static String buildNpmCommandPrefix() {
        return buildNpmRegistryResolverFunction()
            + "NPM_CONFIG_REGISTRY=\"$(koda_resolve_npm_registry)\"\n"
    ...
    public static String buildModelListCommand(boolean enableTrace) {
        return enableTrace ? "KODA_TRACE_NPM_REGISTRY=1 " + MODEL_LIST_COMMAND : MODEL_LIST_COMMAND;
    }
    ...
    /**
     * Returns "openclaude@version" format for installation.
     */
    ...
    /**
     * Strip "openclaude@", "v" prefixes and quotes for bare version comparison.
     */
    ...
    public static boolean isStableVersion(String version) {
        ...
        try {
            OpenClaudeUpdateChecker.parseSemver(version);
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public static List<String> sortAndLimit(List<String> versions) {
        ...
        Collections.sort(unique, OpenclaudeVersionUtils::compareDesc);
    ...
    public static int compareDesc(String a, String b) {
        ...
        try {
            int[] av = OpenClaudeUpdateChecker.parseSemver(a);
            int[] bv = OpenClaudeUpdateChecker.parseSemver(b);
            for (int i = 0; i < 3; i++) {
                if (av[i] != bv[i]) {
    ...
    private static String buildNpmRegistryResolverFunction() {
        return "koda_resolve_npm_registry() {\n"
            + "  default_registry=\"" + DEFAULT_NPM_REGISTRY + "\"\n"
            + "  cn_registry=\"" + CN_NPM_REGISTRY + "\"\n"
            + "  cache_file=\"$HOME/.koda_npm_registry_cache\"\n"
            ...
            + "  cache_registry=\"\"\n"
            + "\n"
            + "  if [ -n \"$KODA_NPM_REGISTRY\" ]; then\n"
            + "    case \"$KODA_NPM_REGISTRY\" in\n"
            + "      http://*|https://*)\n"
            + "        echo \"$KODA_NPM_REGISTRY\"\n"
            + "        ;;\n"
            + "      *)\n"
            ...
            + "        resolved_probe=\"$(curl -m 2 -o /dev/null -s -w '%{http_code}' \"${resolved}openclaude\" 2>/dev/null)\"\n"
            + "      elif command -v wget >/dev/null 2>&1; then\n"
            + "        wget -q -T 2 -t 1 --spider \"${resolved}openclaude\" >/dev/null 2>&1 && resolved_probe=200\n"
            + "      fi\n"
            ...
            + "    if command -v curl >/dev/null 2>&1; then\n"
            + "      npmjs_probe=\"$(curl -m 2 -o /dev/null -s -w '%{http_code}' \"${default_registry}openclaude\" 2>/dev/null)\"\n"
            + "      npmmirror_probe=\"$(curl -m 2 -o /dev/null -s -w '%{http_code}' \"${cn_registry}openclaude\" 2>/dev/null)\"\n"
            + "    elif command -v wget >/dev/null 2>&1; then\n"
            + "      wget -q -T 2 -t 1 --spider \"${default_registry}openclaude\" >/dev/null 2>&1 && npmjs_probe=200\n"
            + "      wget -q -T 2 -t 1 --spider \"${cn_registry}openclaude\" >/dev/null 2>&1 && npmmirror_probe=200\n"
            + "    fi\n"
            + "\n"
            ...
            + "    resolved=\"$default_registry\"\n"
            + "  fi\n"
            + "  printf \"gateway=%s\\nregistry=%s\\n\" \"$gateway\" \"$resolved\" > \"$HOME/.koda_last_npm_registry\"\n"
            + "  echo \"$resolved\"\n"
            + "}\n";
    }
