...
public final class KodaDialogStyler {

    private KodaDialogStyler() {}
    ...
    public static LinearLayout createInlineProgressContent(@NonNull Context context, @StringRes int messageRes) {
        ...
        content.setBackgroundResource(R.drawable.koda_dialog_card_bg);

        ProgressBar progressBar = new ProgressBar(context);
        progressBar.setId(android.R.id.progress);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            progressBar.setIndeterminateTintList(ColorStateList.valueOf(
                ContextCompat.getColor(context, R.color.koda_accent)
            ));
        }
        ...
        messageView.setTextColor(ContextCompat.getColor(context, R.color.koda_on_background));
