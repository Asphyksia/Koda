...
/**
 * Checks the npm registry for newer versions of OpenClaude.
 ...
 */
public class OpenClaudeUpdateChecker {

    private static final String LOG_TAG = "OpenClaudeUpdateChecker";
    private static final long CHECK_INTERVAL_MS = 24 * 60 * 60 * 1000; // 24 hours
    private static final String PREFS_NAME = "openclaude_update";
     ...
     * Requires a bound KodaService to execute shell commands.
     */
    static void check(Context ctx, KodaService service, UpdateCallback cb) {
        check(ctx, service, cb, false);
    }

    static void check(Context ctx, KodaService service, UpdateCallback cb, boolean force) {
        ...
        // Preconditions
        if (!KodaService.isBootstrapInstalled() || !KodaService.isOpenclaudeInstalled()) {
            Log.e(LOG_TAG, "Bootstrap or OpenClaude not installed, skipping");
        ...
        service.executeCommand(OpenclaudeVersionUtils.buildLatestVersionCommand(), result -> {
